...
if [ "$1" = "--no-container" ]; then
  ...
  docker build -t thunor_webpack thunorweb/static
  docker run --rm -v $THUNORHOME/_state/webpack-bundles:/_state/webpack-bundles thunor_webpack || exit $?
  docker-compose run --rm -v $THUNORHOME/_state/webpack-bundles:/thunor/_state/webpack-bundles -v $THUNORHOME/_state/thunor-static:/thunor/_state/thunor-static app python manage.py collectstatic --no-input --ignore thunorweb || exit $?
